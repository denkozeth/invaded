...
/// @creator: denkozeth
/// Special thanks to Pak his Censored collection for inspiring us.
...
contract CommunityTwo is ReentrancyGuard, AdminControl, ERC721 {
     ...
     */
    function deactivate(uint256 messageEndTime_) external adminRequired {
        ...
        returns (string memory)
    {
        ...
        if (tokenId == 1) {
            ...
            string memory community;
            for (uint256 i = 0; i < _communities.length; i++) {
                if (IERC721(_communities[i]).balanceOf(msg.sender) > 0) {
    ...
    {
        require(
            (block.timestamp <= messageEndTime && _tokenIndex >= 2) ||
                msg.sender == owner(),
            "Cannot message"
        );
